...
| [docs/DESIGN.md](DESIGN.md) | Design record: every decision, its reasoning, and which questions are still open. Read it before doing design work. |
| [docs/CONVENTIONS.md](CONVENTIONS.md) | Things like coordinate system axes handedness, units, socket names, exact import/export settings. |
| [docs/CODE_STYLE.md](CODE_STYLE.md) | How the Rust code is written: modern Rust, lifetime rules, crates, dependencies, and API hygiene. Read it before writing or reviewing code. |
...
See [CODE_STYLE.md] — read it before writing Rust code. Prefer Bevy code standard over generic Rust code standards unless enforced by cargo fmt. `cargo fmt` always wins.
