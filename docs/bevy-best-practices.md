...
Original source: https://github.com/tbillington/bevy_best_practices
...
| `Events<E>` | `Messages<M>` | 0.17 |
| `app.add_event::<E>()` | `app.add_message::<M>()` | 0.17 |
| `on_event::<E>()` | `on_message::<M>` | 0.17 |
| `events.iter()` | `messages.read()` | 0.14 |
| `query.for_each(...)` | `for x in &query` or `query.iter()` | 0.13, removed 0.14 |
| `query.get_single()` | `query.single()`, which returns `Result` | 0.16 |
| `query.single()` panicking | `query.single().unwrap()`, or the `Single<T>` param | 0.16 |
| `despawn_recursive()` | `despawn()`, recursive by default | 0.16 |
| `Parent` component, `set_parent(e)` | `ChildOf(e)` relationship, inserted like any component | 0.16 |
| `StateScoped(S)` | `DespawnOnExit(S)` | 0.17 |
...
- Fallible systems, added in 0.16. Systems can return `Result` and use `?`. This project keeps
  the early-return default in the Getter Macros section for entity lookups, and reserves
  `Result` for asset, IO, and parsing failures.

Also note the Getter Macros section predates `Single<T>` and `Populated<T>`, added in 0.16.
Those give the same silent-skip behaviour as the macros, in the signature, with no macro. Use
them where they fit.
